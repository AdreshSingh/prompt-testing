export const assessmentLinks = [
    { name: "Support Team ( SSM ) - Prompt Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-4lzn1cea58&invb=usery45kf7tb" },
    { name: "Delivery Team - Prompt Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-4dt6gt5vxi&invb=usery45kf7tb" },
    { name: "Placement Team - Prompt Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-090a13nsb2&invb=usery45kf7tb" },
    { name: "Video Editing Team - Prompt Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-4gknloc92i&invb=usery45kf7tb" },
    { name: "Content Team - Prompt Testing -", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-g9iqelfcal&invb=usery45kf7tb" },
    { name: "Auditing Team - Prompting Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-u7njippk3p&invb=usery45kf7tb" },
    { name: "B2I Team - Prompting Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-zjvlo0xqh7&invb=usery45kf7tb" },
    { name: "Assessment Team - Prompting Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-ihx9kmm8jv&invb=usery45kf7tb" },
    { name: "Sales Team - Prompt Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-tledj1zddh&invb=userjzsl6u3e" },
    { name: "Program Successor & Influencer Team - Prompting Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-3hrb4c8ze6&invb=userjzsl6u3e" },
    { name: "General Assessment  ( Leaders, Arct, HR, Analyst, Product ) - Prompt Test", link: "https://coderbyte.com/sl-candidate?promo=adda247-cktrp:technical-assessment-bcvtqohwgv&invb=userjzsl6u3e" }
];